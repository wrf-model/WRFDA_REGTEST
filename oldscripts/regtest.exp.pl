...
GetOptions( "compiler=s" => \$Compiler_defined,
            ...
            "j:s" => \$Parallel_compile);

unless ( defined $Compiler_defined ) {
  print "Usage : regtest.pl --compiler=COMPILER --source=SOURCE_CODE.tar --upload=[no]/yes 
...
while (<DATA>) {
     ...
     next if /^#/;
     if ( /^(\D)/ ) {
         ($Arch, $Machine, $Source, $Compiler, $Project, $Queue, $Database, $Baseline) = 
               split /\s+/,$_;
     }
...
unless ($Arch eq "Linux" && $Machine eq "x86_64") {$ENV{J}="-j $Parallel_compile_num"};
...
foreach my $option (sort keys %Compile_options) {
     ...
     # compile all_wrfvar
     printf "Compiling in debug mode, compilation optimizations turned off." unless ( !$Debug );
     ...
     die "The number of exe files is less than 43. \n" if (@exefiles < 43);
    ...
    if ($Arch eq "Linux") {
        if ($Machine eq "x86_64") { # Yellowstone
            if (!-d "/glade/p/work/kavulich/REGRESSION/regtest_compile_logs/$year$mon$mday") {
                mkpath("/glade/p/work/kavulich/REGRESSION/regtest_compile_logs/$year$mon$mday") or die "mkpath failed: $!\n/glade/p/work/kavulich/REGRESSION/regtest_compile_logs/$year$mon$mday";
            }
        } else { # Loblolly
            if (!-d "/loblolly2/kavulich/REGRESSION/regtest_compile_logs/$year$mon$mday") {
                mkpath("/loblolly2/kavulich/REGRESSION/regtest_compile_logs/$year$mon$mday") or die "mkpath failed: $!\n/loblolly2/kavulich/REGRESSION/regtest_compile_logs/$year$mon$mday";
            }
        }
    }
...
if ( ($Arch eq "Linux") && ($Machine eq "x86_64") ) {
    $MainDir = `pwd`;
    chomp($MainDir);
...
$remain_exps_ys[0] = &dig_for_gold ('serial');
$remain_exps_ys[1] = &dig_for_gold ('smpar');
$remain_exps_ys[2] = &dig_for_gold ('dmpar');

print "serial: $remain_exps_ys[0]\n";
print "smpar: $remain_exps_ys[1]\n";
print "dmpar: $remain_exps_ys[2]\n";

# This subroutine determines the number of jobs submitted with a given parallelization
# option, when this option is given to the subroutine as a string.
sub dig_for_gold {
  my ($parname) = @_;
  my $gold = 0;
         foreach my $name (keys %Experiments) {
...
foreach my $name (keys %Experiments) {
#    $remain_par_ops = scalar keys %{$Experiments{$_}{paropt}} 
#        unless ( scalar keys %{$Experiments{$_}{paropt}} < $remain_par_ops ) ; # find the number of different parallelization options
...
if ($Arch eq "AIX") {
    &submit_job_be ;
    } elsif ( ($Arch eq "Linux") && ($Machine eq "x86_64") ) {
    &submit_job_ys ;
    } else {
    &submit_job ;
    }
...
sub new_job_ys {
     ...
     unless ( $par =~ /^(dmpar|smpar|serial)$/ ) {
         print color("red"), colored("Parallelization option '$par' not recognized for task '$nam'\n"), color("reset");
         return undef;
     }
...
sub compare2baseline {
     ...
     return -3 unless ( -e "$name/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler");
     return -4 unless ( -e "$Baseline/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler");

     my @output = `$diffwrfpath $name/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler $Baseline/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler`;
    ...
    foreach my $name (keys %Experiments) {
        ...
        foreach my $par (sort keys %{$Experiments{$name}{paropt}}) {
            ...
            if (defined $rc) { 
                ...
                $Experiments{$name}{paropt}{$par}{compare} = sprintf colored("%-10s",'red on_yellow blink'), "error";
            ...
            rename "$name/wrfvar_output", "$name/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler";

            # Compare the wrfvar_output with the BASELINE:

            unless ($Baseline =~ /none/i) {
                if (compare ("$name/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler","$Baseline/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler") == 0) {
                    $Experiments{$name}{paropt}{$par}{compare} = "match";
                } else {
                    my $baselinetest = &compare2baseline ($name,$par);
                    my %compare_problem = (
                        1       => "diff",
                        -1      => "ERROR",
                        -2      => "ERROR: diffwrf comparison failed",
                        -3      => "ERROR: WRFDA failed (output missing)",
                        -4      => "ERROR: Baseline missing",
                        -5      => "ERROR: Could not open output and/or baseline",
                    );
                    
                    if ( $baselinetest ) {
                        $Experiments{$name}{paropt}{$par}{compare} =
                            sprintf colored("%-10s",'red on_yellow blink'), $compare_problem{$baselinetest};
                    } elsif ( $missvars ) {
    ...
    while ($remain_exps > 0) {    # cycling until no more experiments remain
         ...
         foreach my $name (keys %Experiments) {
             ...
             foreach my $par (sort keys %{$Experiments{$name}{paropt}}) {
                 ...
                 unless (defined $Experiments{$name}{paropt}{$par}{starttime}) { # This job exited mysteriously: killed or time out.
...
sub submit_job_ys {
  while ($remain_par_ops > 0) {
    print $remain_exps_ys[$remain_par_ops-1];
    while ($remain_exps_ys[$remain_par_ops-1] > 0) {    # cycling until no more experiments remain
         print $remain_exps_ys[$remain_par_ops-1];
         #Loop over parallelization options (dmpar jobs first, then serial). This prevents conflicts within individual directories.
...
#             die "THIS IS THE END!!!!\n";
             ...
             foreach my $par (sort keys %{$Experiments{$name}{paropt}}) {
#                 print "$par eq 'dmpar':".($par eq "dmpar")."\n";
#                 print "$par eq 'smpar' && $remain_par_ops < 3:".($par eq "smpar" && $remain_par_ops < 3)."\n";
#                 print "$par eq 'serial' && $remain_par_ops < 2:".($par eq "serial" && $remain_par_ops < 2)."\n";
                 next unless ( ($par eq "dmpar") || 
                               ($par eq "smpar" && $remain_par_ops < 3) ||
                               ($par eq "serial" && $remain_par_ops < 2) );
                 next if ( $Experiments{$name}{paropt}{$par}{status} eq "done"  ||      # go to next job if it is done already..
                           $Experiments{$name}{paropt}{$par}{status} eq "killed" ||
                           $Experiments{$name}{paropt}{$par}{status} eq "error" );
...
#                     next if $Experiments{$name}{status} eq "close";      #  skip if this experiment close to submit job .
                     ...
                     if (defined $rc) {
                         ...
                         $Experiments{$name}{paropt}{$par}{status} = sprintf colored("%-10s",'red on_yellow blink'),"error";
                         $Experiments{$name}{paropt}{$par}{compare} = sprintf colored("%-10s",'red on_yellow blink'), "job not submitted";
                         $remain_exps_ys[$remain_par_ops-1] -- ;
                         next;   # Can not submit this job.
                     }
                 }
                 ...
                 unless (defined $Experiments{$name}{paropt}{$par}{starttime}) { # This job was exit mysteriously: killed or time out.
                     ...
                     $Experiments{$name}{paropt}{$par}{compare} = sprintf colored("%-10s",'red on_yellow blink'),"unknown";
                     delete $Experiments{$name}{paropt}{$par}{jobid};       # Delete the jobid.
                     $remain_exps_ys[$remain_par_ops-1] -- ;                                      # Done with this job, de-increment remaining experiments
                     $Experiments{$name}{paropt}{$par}{status} = sprintf colored("%-10s",'red on_yellow blink'),"killed";
                 } else {
                     $Experiments{$name}{paropt}{$par}{walltime} =
                          $Experiments{$name}{paropt}{$par}{endtime} - $Experiments{$name}{paropt}{$par}{starttime};
                 ...
                 # Compare against the baseline
                     ...
                     unless ($Baseline =~ /none/i) {
                         print "\nComparing '$name/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler' to '$Baseline/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler'" ;
                         if (compare ("$name/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler","$Baseline/wrfvar_output.$Arch.$Machine.$name.$par.$Compiler") == 0) {
                             $Experiments{$name}{paropt}{$par}{compare} = "match";
                         } else {
                             my $baselinetest = &compare2baseline ($name,$par);
                             ...
                             print "$compare_problem{$baselinetest}";
...
#                 }
  ...
  print "$remain_par_ops";
  $remain_par_ops -- ;
  print "$remain_par_ops";
#  $remain_exps = scalar keys %Experiments;
  }
}
...
###########################################################################################
#ARCH    MACHINE  SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
AIX      N/A      /blhome/jliu/regtest/WRFDA.tar          XLF         64000510  share /ptmp/jliu/regtest/WRFDA-data-EM  /mmm/users/wrfhelp/data/BASELINE
...
###########################################################################################
#ARCH      MACHINE     SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Linux      x86_64      /glade/p/work/kavulich/REGRESSION/WRFDA_V3.4.tar        ifort         P64000510  caldera   /glade/p/work/kavulich/REGRESSION/WRFDA-data-EM    /glade/p/work/kavulich/REGRESSION/BASELINE.NEW
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
1        tutorial_xinzhang           16      16            dmpar|serial
2        cv3_guo                     16      16            dmpar|serial
3        t44_liuz                    16      16            dmpar|serial
4        radar_meixu                 16      16            serial|dmpar
5        cwb_ascii                   16      16            serial|dmpar
6        afwa_t7_ssmi                16      16            serial|dmpar
7        t44_prepbufr                16      16            serial|dmpar
8        ASR_prepbufr                16      16            serial|dmpar
9        cwb_ascii_outerloop_rizvi   16      16            serial|dmpar
10       sfc_assi_2_outerloop_guo    16      16            serial|dmpar
11       outerloop_bench_guo         16      16            serial|dmpar
12       outerloop_ztd_bench_guo     16      16            dmpar|serial
13       tutorial_xinzhang_kmatrix   16      16            serial|dmpar
14       tutorial_xinzhang_rttov     16      16            serial|dmpar
############################################################################################
#ARCH      MACHINE   SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Linux      i686      /loblolly/kavulich/wrfda.tar        ifort         64000510  share   /loblolly/kavulich/WRFDA-data-EM    /loblolly/kavulich/BASELINE
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
...
###########################################################################################
#ARCH      MACHINE   SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Linux      i686      /loblolly2/kavulich/REGRESSION/WRFDA34/WRFDA_V3.4.tar        gfortran         64000510  share   /loblolly2/kavulich/REGRESSION/WRFDA-data-EM-small    /loblolly2/kavulich/REGRESSION/BASELINE.NEW.SMALL
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
...
###########################################################################################
#ARCH      MACHINE   SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Linux      i686      /loblolly2/kavulich/REGRESSION/WORKDIR/tiso_fix/wrfda_tiso_simple_message.tar        pgi         64000510  share   /loblolly2/kavulich/REGRESSION/WRFDA-data-35-small    /loblolly2/kavulich/REGRESSION/BASELINE.NEW.SMALL
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
1        tutorial_xinzhang           8       8            dmpar|serial
2        cv3_guo                     8       8            dmpar|serial
3        t44_liuz                    8       8            dmpar|serial
...
###########################################################################################
#ARCH      MACHINE     SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Darwin     x86_64      wrfda.tar        pgi         64000510  share   /data3/mp/wrfhelp/data//WRFDA-data-EM    /data3/mp/wrfhelp/data//BASELINE
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
...
###########################################################################################
#ARCH      MACHINE     SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Darwin     x86_64      wrfda.tar        g95         64000510  share   /data3/mp/wrfhelp/data//WRFDA-data-EM    /data3/mp/wrfhelp/data//BASELINE
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
