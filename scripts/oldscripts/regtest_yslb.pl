...
# Updates : January 2013, adapted for Yellowstone, added bit-for-bit serial/dmpar test (Mike Kavulich)
#
#
...
unless ( defined $Compiler_defined ) {
  print "Usage : regtest.pl --compiler=pgi --source=wrfda.tar/SVN --upload=[no]/yes --exec=[no]/yes\n";
  print "        source: Specify source directory\n";
...
die "Compiler $Compiler_defined is not supported on this $Arch $Machine machine. \n" unless (keys %Experiments) > 0 ; 
# Get the codes:
...
sub compare2baseline {
     ...
     return -3 unless ( -e "$name/wrfvar_output.$Arch.$name.$par.$Compiler");
     return -3 unless ( -e "$Baseline/wrfvar_output.$Arch.$name.$par.$Compiler");
     ...
     my @output = `$diffwrfpath $name/wrfvar_output.$Arch.$name.$par.$Compiler $Baseline/wrfvar_output.$Arch.$name.$par.$Compiler`;
...
###########################################################################################
#ARCH      MACHINE     SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Linux      x86_64      /glade/p/work/kavulich/REGRESSION/WRFDA_V3.4.tar        ifort         P64000510  regular   /glade/p/work/kavulich/REGRESSION/WRFDA-data-EM    /glade/p/work/kavulich/REGRESSION/BASELINE.NEW
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
1        tutorial_xinzhang           16      16           serial|dmpar
2        cv3_guo                     16      16            serial|dmpar
3        t44_liuz                    16      16            serial|dmpar
...
#12       outerloop_ztd_bench_guo     16      16            serial|dmpar
...
###########################################################################################
#ARCH      MACHINE   SOURCE     COMPILER    PROJECT   QUEUE   DATABASE                             BASELINE
Linux      i686      /loblolly2/kavulich/REGRESSION/WRFDA34/WRFDA_V3.4.tar        pgi         64000510  share   /loblolly2/kavulich/REGRESSION/WRFDA-data-EM-small    /loblolly2/kavulich/REGRESSION/BASELINE.NEW.SMALL
#INDEX   EXPERIMENT                  CPU     OPENMP       PAROPT
1        tutorial_xinzhang           8       8            serial|dmpar
2        cv3_guo                     8       8            serial|dmpar
3        t44_liuz                    8       8            serial|dmpar
